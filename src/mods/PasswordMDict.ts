...
import { CHAR_SETS  as C} from "../utils/constants";
import { FALLBACK_WORDLIST } from "./FALLBACK_WORDLIST";
...
export function loadEFFWordlist(
  ...
  // ── Validate result ─────────────────────────────────────────────────────────
  if (words.length < WMLIMIT) {
    throw new Error(
      `loadEFFWordlist: file "${absolutePath}" yielded only ${words.length} valid word(s). ` +
        `Expected an EFF wordlist with at least ${WMLIMIT} entries.`,
    );
  }
