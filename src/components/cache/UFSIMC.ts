...
import { Random, Hash } from "../../core/index";
...
class UFSIMC extends EventEmitter {
  ...
  private initializeEncryption(): void {
    try {
      if (process.env.ENC_SECRET_KEY) {
      ...
      } else if (process.env.ENC_SECRET_SEED && process.env.ENC_SECRET_SALT) {
        const itSalt = process.env.ENC_SECRET_SALT;
        let ck = process.env.ENC_SECRET_SEED;
        for (let i = 0; i < CONFIG.KEY_ITERATIONS; i++) {
          ck = Hash.hmac(ck, itSalt);
        }
        this.encryptionKey = Buffer.from(ck.substring(0, 64), "hex");
      } else {
        const isSilent =
        ...
        this.encryptionKey = Buffer.from(
          Random.getRandomBytes(CONFIG.KEY_LENGTH),
        );
      }
      ...
      this.logger.error("security", "Failed to initialize encryption:", error);
  ...
  private fastEncrypt(data: string): {
    ...
    try {
      ...
      const encHex = Bridge.encryptRaw(dataBuf, this.encryptionKey, "aes");
   ...
   */
  private fastDecrypt(encrypted: string, iv: string, authTag: string): string {
    try {
      const partsStr = `${iv}:${authTag}:${encrypted}`;
      const decryptedHex = Bridge.decryptRaw(
        partsStr,
        this.encryptionKey,
        "aes",
      );
      ...
      const decBytes = new Uint8Array(decMatches.map((b) => parseInt(b, 16)));
  ...
  private calculateChecksum(data: string): string {
    return Bridge.sha256(data).substring(0, 16);
  }
  ...
  public async set(
    ...
    try {
      ...
      let encrypted: string, iv: string, authTag: string, checksum: string;
      ...
      checksum = this.integrityCheck ? this.calculateChecksum(serialized) : "";
        ...
        try {
          ...
          if (originalKey) {
            data[originalKey] = {
              ...
              metadata: (node.entry as UltraMemoryCacheEntry).metadata,
              tags: Array.from((node.entry as UltraMemoryCacheEntry).tags),
              expiresAt: (node.entry as UltraMemoryCacheEntry).expiresAt,
            };
          }
        } catch (error) {
          this.logger.warn(
  ...
  private cleanupIndexes(
    ...
    if (entry.priority >= 0 && entry.priority < this.priorityQueues.length) {
  ...
  private async rotateEncryptionKey(): Promise<void> {
    try {
      const newKey = Buffer.from(Random.getRandomBytes(CONFIG.KEY_LENGTH));
