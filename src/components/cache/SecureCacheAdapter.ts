...
import { XyPrissSecurity as XyPrissJS } from "../../core/index";
...
export class SecureCacheAdapter extends EventEmitter {
  ...
  private initializeStats(): void {
    this.stats = {
      ...
      redis:
        this.config.strategy === "redis" || this.config.strategy === "hybrid"
          ? {
              connected: false,
              commandsProcessed: 0,
              operations: 0,
  ...
  private initializeMasterKey(): void {
    ...
    if (!SecureCacheAdapter.sharedMasterEncryptionKey) {
      SecureCacheAdapter.sharedMasterEncryptionKey = XyPrissJS.generateAPIKey({
        prefix: "xycache",
        randomPartLength: 32,
      });
    }
    this.masterEncryptionKey =
      SecureCacheAdapter.sharedMasterEncryptionKey || "";
  }
  ...
  private async performHealthCheck(): Promise<void> {
    try {
      ...
      if (thresholds) {
        if (
          memoryStats.memoryUsage.percentage > (thresholds.memoryUsage || 90)
        ) {
          this.emit("performance_alert", {
  ...
  private generateKey(key: string): string {
    ...
    // Create deterministic hash of the key using Go bridge
    const hashedKey = Bridge.sha256(key).substring(0, 16);

    return `XyPriss:v2:${hashedKey}:${key}`;
  }
  ...
  private isHotData(key: string): boolean {
    ...
    return (
      meta.accessCount >= threshold && now - meta.lastAccessed < timeWindow
    );
  }
    ...
    try {
      ...
      if (
        ...
        if (redisResult !== null) {
          ...
          if (this.config.strategy === "hybrid" && this.isHotData(cacheKey)) {
  ...
  public async set<T = any>(
    ...
    try {
      ...
      const shouldStoreInMemory =
        this.config.strategy === "memory" || this.config.strategy === "hybrid";
  ...
  private async updateRedisStats(): Promise<void> {
    ...
    try {
      ...
      if (infoData.uptime_in_seconds) {
        this.stats.redis.uptime = parseInt(infoData.uptime_in_seconds) * 1000; // Convert to ms
      }
